...
async fn aws_login(mut config: config::CliConfig) {
    print!("Enter token code for MFA ({}): ", config.get_mfa());
    io::stdout().flush().ok().expect("Could not flush stdout");

    let mut token_code = String::new();
    io::stdin().read_line(&mut token_code).unwrap();
    ...
    let result = sts.get_session_token(get_token_request).await.unwrap();

    config.set_session_token(result.credentials.unwrap());
}
